...
$(".catS").click(function (e) { 
    //$("#sidebar_mobile").removeClass("shw-rside");
    console.log("Borra")
...
const imprimePrecuenta = ()=>{
    ...
    if($("#print_pre").val() == 1){
        (async() => {
            ...
            const { value: formValues } = await Swal.fire({
                ...
                html: ` 
                    ...
                    <select id="piso" value="" class="swal2-input">
            ...
            if (formValues) {
                ...
                if(data.piso != ""){
                    ...
                    Swal.fire({
                        title: 'Favor de ingresar un piso', 
                        html: 'No se puede imprimir, seleccione el piso', 
                        icon: 'warning'
                    })
                }
            }
        ...
        // Swal.fire({
        //     title: 'En que piso deseas imprimir?',
        //     html: '',
        //     showCancelButton: true,
        //     showConfirmButton: true, 
        //     showDenyButton: true
        // }).then((callback)=>{
        //   if(callback.dismiss == 'cancel'){        
        //         window.open($("#url").val()+'venta/impresion_precuenta/'+id_pedido+'/2/'+JSON.stringify(data_Selected),'_blank');
        //     }else{
        //         if(callback.value == true){
        //             window.open($("#url").val()+'venta/impresion_precuenta/'+id_pedido+'/1/'+JSON.stringify(data_Selected),'_blank');
        //         }
        //     }
        // })
    }else{
        ...
        if($('#print_com').val() == 1){
            ...
            if(($('#codtipoped').val() == 1 && $('#codimpcomandamesa').val() == 1) || $('#codtipoped').val() == 2 || $('#codtipoped').val() == 4){
                ...
                for(y=0; y < cont.length; y++){
                    console.log(cont.length + ' AREAS  A IMPRIMIR')
...
// Sin embargo, para que funcione, vamos a utilizar el servicio gratuito de cors-anywhere para evitar este
        var proxy = 'https://cors-anywhere.herokuapp.com/';
                    ...
                    $url = 'http://'+$('#pc_ip').val()+'/imprimir/comanda.php?data=';
                    $json = JSON.stringify(nuevopedido);
                    $id_ped = $("#codped").val();
                    registraImpresion(nuevopedido.nombre_imp, "COMANDA", $id_ped, $url, $json);
                    $imp = nuevopedido.nombre_imp;
                    $.ajax({
                        type: "GET",
                        url: proxy + 'http://'+$('#pc_ip').val()+'/imprimir/comanda.php?data='+JSON.stringify(nuevopedido),
                        success: () => {
                        },
                    ...
                    console.log("Impreso a "  + nuevopedido.nombre_imp)
                    var nuevoarray = new Array();
                } 
...
var clickSendWhatsapp = function(id) {
    ...
    if(!num_cliente){
        ...
        console.log('https://wa.me/51'+num_cliente+'?text='+meg_com);
    }
};

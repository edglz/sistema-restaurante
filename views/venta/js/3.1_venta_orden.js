                    ...
                    if (data.length != 0) {
                        ...
                        //Receiving the result of search here
                            $.each(data, function(i, item) {
                                $('#list-productos')
                                    .append(
                                        $('<article class="product '+prodwidth+' animated fadeIn" onclick="add('+item.id_areap+','+item.id_pres+',\''+item.pro_nom+'\',\''+item.pro_pre+'\','+item.pro_cos+',\''+item.Impresora.nombre+'\', \''+item.is_combo+'\');"/>')
                                        .append(
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function (dato) {
                            ...
                            if(venta.tipo_pedido == 4){
                                var html_footer = $("#url").val()+'venta/venta_portero';
                            }
                            if($('#print_cpe').val() == 1){
                                // imprimir directo 
                                 // imprimir directo 
                                 $id_ped = $("#codped").val();
                                 $url = $("#url").val()+'informe/venta_all_imp_/'+cod_ven;
                                 $caja = "CAJERO";
                                 $tipo = "COMPROBANTE DE VENTA";
                                var linkpdf     = '<a onclick="rec(\''+$caja+'\',\''+$tipo+'\',\''+$id_ped+'\',\''+$url+'\')" href="'+$("#url").val()+'informe/venta_all_imp_/'+cod_ven+'" target="_blank" class="link rec"><i class="mdi mdi-receipt text-muted"></i></a>';
                                var linkpdff    = '<a onclick="rec(\''+$caja+'\',\''+$tipo+'\',\''+$id_ped+'\',\''+$url+'\')" class="link rec"  href="'+$("#url").val()+'informe/venta_all_imp_/'+cod_ven+'"target="_blank"><small>Imprimir comprobante</small></a>';
                                var linkrep     = '<a onclick="rec(\''+$caja+'\',\''+$tipo+'\',\''+$id_ped+'\',\''+$url+'\')" class="link rec" href="'+$("#url").val()+'venta/impresion_reparto/'+cod_ven+'" target="_blank"><i class="fas fa-print text-muted"></i></a>';
                                var linkrepp    = '<a onclick="rec(\''+$caja+'\',\''+$tipo+'\',\''+$id_ped+'\',\''+$url+'\')" class="link rec" href="'+$("#url").val()+'venta/impresion_reparto/'+cod_ven+'" target="_blank"><small>Ticket reparto</small></a>';
        ...
        success: function (data) {
            $.each(data, function(i, item) {
                $('#list-catgrs')
                    ...
                    .append($('<li class="nav-item list-salones m-t-10">').append($(`
                    <a class="nav-link" data-toggle="list" href="javascript:void(0)" onclick="listarProductos('${item.id_catg}', ${false});"/ >
                    `).html(item.descripcion))   );
            });
        }
    });
...
var listarCategoriasMovil = function(){
    ...
    $('#list-catgrs-movil').html('<a class="catP b-0 list-group-item list-group-item-action active" style="border-radius: 0px;" data-toggle="list" href="javascript:void()" onclick="listarProdsMasVend();"><b>FAVORITOS</b></a>');
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function(i, item) {
                $('#list-catgrs-movil')
                    .append(
                        $('<a class="catS b-0 list-group-item list-group-item-action" data-toggle="list" href="javascript:void()" onclick="listarProductos('+item.id_catg+');"/>')
                        .html(item.descripcion)
...
var listarProductos = function(id_catg, mobile = true){
    if(mobile == true){
        esconde_menu();
    }
    $('#buscar_producto').val('');
...
const imprimePrecuenta = ()=>{
    var id_pedido = $("#codped").val();
    if($("#print_pre").val() == 1){
        (async() => {

            const { value: formValues } = await Swal.fire({
                title: 'Seleccione el piso que desea imprimir',
                html: ` 
                    <div class="row">
                        <div class="col-6">
                            <button class="btn btn-dark btn-rounded piso col-12 p-4 mb-2" id="piso1" data-selected="false"  data-piso="1">Piso #1</button>
                        </div>
                        <div class="col-6">
                            <button class="btn btn-danger btn-rounded piso col-12 p-4 mb-2" id="piso2" data-selected="false"  data-piso="2">Piso #2</button>
                        </div>
                        <div class="col-12">
                        <button class="btn btn-info piso btn-rounded col-12 p-4 mb-2" id="cajero" data-selected="false"  data-piso="3">Cajero</button>
                        </div>
                    </div>
                    `,
                focusConfirm: false,
        ...
        window.open($("#url").val()+'venta/impresion_precuenta/'+ $('#codped').val()+'/1/'+JSON.stringify(data_Selected)+'','_blank');
    }
    $(".piso").on("click", (e)=>{
        let id = e.currentTarget.id
        let el = document.getElementById(id)
        let piso = el.dataset.piso;
        let data = {
            'piso': piso,
        ...
        if(data.piso != ""){
            ...
            window.open($("#url").val()+'venta/impresion_precuenta/'+id_pedido+'/'+data.piso+'/'+JSON.stringify(data_Selected),'_blank');
            $url = $("#url").val()+'venta/impresion_precuenta/'+id_pedido+'/'+data.piso+'/';
            let $imp = ''
            if(data.piso == "1"){
                $imp = "PRECUENTA PISO 1";
            }else{
                if(data.piso == "2"){
                    $imp = "PRECUENTA PISO 2";
                }else{
                    $imp = "CAJERO";
                }
            }
            $id_ped = $("#codped").val();
            $json = JSON.stringify(data_Selected);
            registraImpresion($imp, 'PRECUENTA', $id_ped, $url, $json);
            Swal.close()
        }
    })
}
...
const deleteListProductos = (id_pedido) =>{
    if(data_Selected.length > 0){
        Swal.fire({
            ...
            +'<form><input class="form-control text-center w-50" type="text" id="codigo_seguridad" autocomplete="off"/></form>',
            showCancelButton: true,
            icon: 'warning',
            showCancelButton: true,
...
var add = function(id_areap,id_pres,nombre_prod,pres_prod,precio,nombre_imp, combo = 'a'){
    $("#nvo-ped").css('display','block');
    $(".bc").css('display','block');
    pedido.registrar({
        combo: combo,
        combos_detalle: new Array(),
...
var rec = (imp, tipo, id_ped, url)=>{
    registraImpresion(imp, tipo, id_ped, url, '');
}
$("#btn-confirmar").on("click", function(){
    ...
    if(pedido.detalle.items.length == 0)
    {
        ...
        if($('#print_com').val() == 1){
            ...
            if(($('#codtipoped').val() == 1 && $('#codimpcomandamesa').val() == 1) || $('#codtipoped').val() == 2 || $('#codtipoped').val() == 4){
                // nueva funcionalidad        
                var area_c = [] 
                let arr = pedido.detalle.items
                var iterator = 0
                arr.forEach((value, index, array)=>{
                    if(value.combo == 'a'){
                        let ajax = get_product(value.producto_id)
                        let json = ajax.responseJSON;
                        if(json.data.length > 0){
                            for(let xz of json.data){
                                pedido.detalle.items[iterator].combos_detalle.push(xz)
                                area_c.push(parseInt(xz.area.id_areap))
                            }
                        }else{
                            area_c.push(parseInt(value.area_id))
                        }
                    }else{
                        area_c.push(parseInt(value.area_id))
                    }
                ...
                const dataArr = new Set(area_c);
                let result = [...dataArr];
                area_c = result
                arr = pedido.detalle.items
                area_c.forEach((value, key, set)=>{
                    console.log(value)
                    ...
                    arr.forEach((v, k, s)=>{
                        if(v.area_id == value){
                            nuevo_array.push(v);
                            nombre_impresora = v.nombre_imp;
                        }
                        for(let xc of v.combos_detalle){
                            if(xc.area.id_areap == value){
                                nuevo_array.push(xc);
                                nombre_impresora = xc.impresora.nombre;
                            }
                        }
                    })
                    var $id_ped = $("#codped").val();
                    var nuevopedido = {
...
var impPreCuenta = function(id_pedido,id_mesa,estado, piso){
    $.ajax({
...
var facturar = function(id_pedido,dividir_cuenta){
    ...
    $("#modal-facturar").modal('show');
    var elements = document.getElementsByName("tipo_doc");
...
function get_product(id){
    return $.ajax({
        type: "POST",
        url: $("#url").val() + "venta/info_product/" + id,
        dataType: "json",
        cache: false,
        async: !1,
    });
